...
async function fetchChatAndResponsibleEvents(subdomain, dateFrom, dateTo) {
  ...
  // Fetch responsible changes (same period)
  const respParams = {
  ...
  console.log('📨 Fetching chat events + responsible changes...');
  ...
  console.log(`📨 Chat events: ${chatEvents.length}, Responsible changes: ${respEvents.length}`);
...
async function buildSLAData(subdomain, options = {}) {
  ...
  // Group chat events by lead
  const chatByLead = {};
  chatEvents
    .filter(e => e.entity_type === 'lead' && leadIds.has(e.entity_id))
    .forEach(e => {
      if (!chatByLead[e.entity_id]) chatByLead[e.entity_id] = [];
      chatByLead[e.entity_id].push(e);
    });
  ...
  return {
    ...
    pending_leads: filteredPending.slice(0, 80),
  };
}
