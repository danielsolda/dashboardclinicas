...
async function buildSLAData(subdomain, options = {}) {
  ...
  // Build a map of lead → latest closed timestamp
  // talk_closed/conversation_answered events can have entity_type = lead, contact, talk, etc.
  // We try ALL resolution strategies to find the lead
  const closedTypes = new Set(['talk_closed', 'conversation_answered']);
  const leadClosedTimes = {}; // leadId → [timestamp, timestamp, ...] all closed times

  chatEvents.filter(e => closedTypes.has(e.type)).forEach(e => {
    let lid = null;
    // Strategy 1: direct lead
    if (e.entity_type === 'lead' && leadIds.has(e.entity_id)) {
      lid = e.entity_id;
    }
    // Strategy 2: contact → lead map
    if (!lid && e.entity_type === 'contact') {
      const mapped = contactToLead[e.entity_id];
      if (mapped && leadIds.has(mapped)) lid = mapped;
    }
    // Strategy 3: try entity_id against contact map regardless of entity_type
    if (!lid) {
      const mapped = contactToLead[e.entity_id];
      if (mapped && leadIds.has(mapped)) lid = mapped;
    }
  ...
  /**
   * Check if there was a close event for this lead at or after the given timestamp.
   */
