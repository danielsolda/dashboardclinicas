...
async function buildSLAData(subdomain, options = {}) {
  ...
  // Exclude leads that have talk_closed or conversation_answered — conversation already resolved
  const closedTypes = new Set(['talk_closed', 'conversation_answered']);
  let excludedCount = 0;
  for (const lid of Object.keys(chatByLead)) {
    if (chatByLead[lid].some(e => closedTypes.has(e.type))) {
      delete chatByLead[lid];
      excludedCount++;
    }
  }
  console.log(`📊 Excluded ${excludedCount} leads (talk_closed/conversation_answered), ${Object.keys(chatByLead).length} remaining`);
