...
async function buildSLAData(subdomain, options = {}) {
  ...
  const chatByLead = {};
  const entityToLead = {}; // entity_id → lead_id (from successfully resolved chat events)
  ...
  chatEvents.forEach(e => {
    ...
    if (lid) {
      ...
      entityToLead[e.entity_id] = lid; // remember: this entity_id belongs to this lead
      mapped++;
    } else {
      unmapped++;
    }
  ...
  console.log(`📊 Entity→Lead reverse map: ${Object.keys(entityToLead).length} unique entity IDs`);

  // Map talk_closed / conversation_answered events to leads.
  // Close events can have entity_type='talk','contact','lead' etc.
  // We use EVERY strategy: direct lead, contactToLead, AND the reverse entityToLead map
  // built from successfully mapped chat events (same contact = same lead).
  const closedTypes = new Set(['talk_closed', 'conversation_answered']);
  const leadClosedTimes = {}; // leadId → [timestamp, ...]

  let closeMatched = 0, closeUnmatched = 0;
  chatEvents.filter(e => closedTypes.has(e.type)).forEach(e => {
    let lid = null;
    // Strategy 1: direct lead ID
    if (e.entity_type === 'lead' && leadIds.has(e.entity_id)) {
      lid = e.entity_id;
    }
    // Strategy 2: contact → lead
    if (!lid) {
      const m = contactToLead[e.entity_id];
      if (m && leadIds.has(m)) lid = m;
    }
    // Strategy 3: reverse entity map (the same entity_id was used by a chat event we already mapped)
    if (!lid) {
      const m = entityToLead[e.entity_id];
      if (m) lid = m;
    }
    // Strategy 4: created_by user — find lead where this user is responsible and has recent chat
    // (fallback, less precise but catches remaining cases)
    if (!lid && e.created_by) {
      // Look for leads where this user has chat events within 60s of the close event
      for (const [leadIdStr, evts] of Object.entries(chatByLead)) {
        const hasNearby = evts.some(ce =>
          ce.created_by === e.created_by &&
          Math.abs(ce.created_at - e.created_at) < 120
        );
        if (hasNearby) { lid = Number(leadIdStr); break; }
      }
    }

    if (lid) {
      if (!leadClosedTimes[lid]) leadClosedTimes[lid] = [];
      leadClosedTimes[lid].push(e.created_at);
      closeMatched++;
    } else {
      closeUnmatched++;
    }
  ...
  for (const lid of Object.keys(leadClosedTimes)) {
    leadClosedTimes[lid].sort((a, b) => a - b);
  }
  console.log(`📊 Close events: ${closeMatched} matched to leads, ${closeUnmatched} unmatched`);
  console.log(`📊 Leads with closed conversations: ${Object.keys(leadClosedTimes).length}`);

  function wasClosedAfter(leadId, ts) {
    const times = leadClosedTimes[leadId];
    if (!times) return false;
    return times.some(t => t >= ts);
  }
  ...
  for (const [leadIdStr, evts] of Object.entries(chatByLead)) {
    ...
    for (const evt of evts) {
      ...
      if (isIncoming) {
        ...
        // Check if conversation was closed between the incoming and this outgoing
        if (wasClosedAfter(leadId, lastIncoming.created_at) && 
            leadClosedTimes[leadId]?.some(t => t >= lastIncoming.created_at && t <= evt.created_at)) {
          // Closed between incoming and outgoing → don't count this pair
          lastIncoming = null;
          continue;
        }
    ...
    // Pending: incoming without outgoing response
    // Skip if conversation was closed/answered AFTER the last incoming message
    if (lastIncoming) {
      if (wasClosedAfter(leadId, lastIncoming.created_at)) {
        // Conversation was closed after this incoming → not pending
        continue;
      }
