...
async function buildSLAData(subdomain, options = {}) {
  ...
  // ─── Build unified timeline per lead ───────────────
  // Include close events (talk_closed, conversation_answered) in the lead timeline
  // so we can check: what is the LAST event for this lead?
  const closedTypes = new Set(['talk_closed', 'conversation_answered']);

  // Collect all entity_ids per lead (from chat events + contacts)
  const leadEntityIds = {};
  for (const [lidStr, evts] of Object.entries(chatByLead)) {
    leadEntityIds[lidStr] = new Set(evts.map(e => e.entity_id));
    const lead = leadMap[Number(lidStr)];
    if (lead?._embedded?.contacts) {
      lead._embedded.contacts.forEach(c => leadEntityIds[lidStr].add(c.id));
    }
  }

  // Group close events by entity_id
  const closeByEntity = {};
  chatEvents.filter(e => closedTypes.has(e.type)).forEach(e => {
    if (!closeByEntity[e.entity_id]) closeByEntity[e.entity_id] = [];
    closeByEntity[e.entity_id].push(e);
  });

  // For each lead, build full timeline: chat msgs + close events from matching entity_ids
  const leadFullTimeline = {};
  for (const [lidStr, evts] of Object.entries(chatByLead)) {
    const entityIds = leadEntityIds[lidStr];
    // Start with chat messages
    const timeline = [...evts];
    // Add close events from any matching entity_id
    if (entityIds) {
      for (const eid of entityIds) {
        const closes = closeByEntity[eid];
        if (closes) timeline.push(...closes);
      }
    }
    timeline.sort((a, b) => a.created_at - b.created_at);
    leadFullTimeline[lidStr] = timeline;
  }

  // Stats
  let leadsLastClosed = 0;
  for (const [lidStr, timeline] of Object.entries(leadFullTimeline)) {
    const last = timeline[timeline.length - 1];
    if (last && closedTypes.has(last.type)) leadsLastClosed++;
  }
  console.log(`📊 Leads where LAST event is close: ${leadsLastClosed} of ${Object.keys(chatByLead).length}`);
  ...
  for (const [leadIdStr, fullTimeline] of Object.entries(leadFullTimeline)) {
    const leadId = Number(leadIdStr);
    const lead = leadMap[leadId];
    if (!lead) continue;

    // Walk the timeline: only incoming, outgoing, talk_closed, conversation_answered
    // Close events reset the pending incoming (conversation resolved without chat response)
    // Valid incoming→outgoing pairs ALWAYS count for SLA regardless of later closes
    let lastIncoming = null;

    for (const evt of fullTimeline) {
      // Close event → conversation resolved, cancel any pending incoming
      if (closedTypes.has(evt.type)) {
        lastIncoming = null;
        continue;
      }
      ...
      if (isIncoming) {
        ...
        // Response found — pair it (always counts for SLA)
    ...
    // After walking the full timeline:
    // If lastIncoming is set → there's an unanswered incoming with no close after it → PENDING
    // If lastIncoming is null → either responded or closed → NOT pending
    if (lastIncoming) {
      const responsibleId = getResponsibleAt(leadId, lastIncoming.created_at);
