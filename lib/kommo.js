...
async function api(subdomain, method, path, params = {}, body = null) {
  ...
  try {
    ...
    const detail = respBody && typeof respBody === 'object'
      ? (respBody.title || respBody.detail || JSON.stringify(respBody).slice(0, 200))
      : (typeof respBody === 'string' ? respBody.slice(0, 200) : '');
    const e = new Error(`Kommo ${method} ${path} → ${status || 'erro de rede'}${detail ? ' · ' + detail : ''}`);
    e.status = status;
    e.kommoPath = path;
    e.kommoBody = respBody;
    throw e;
  }
}
...
async function getFiltersData(subdomain) {
  ...
  // If pipelines fail, the dashboard can't do anything useful — propagate the error.
  if (pRes.status === 'rejected') throw pRes.reason;

  const allPipelines = pRes.value || [];
  const users        = uRes.status === 'fulfilled' ? (uRes.value || []) : [];
  const sourceField  = sRes.status === 'fulfilled' ? sRes.value : null;

  const warnings = [];
  if (uRes.status === 'rejected') {
    console.error(`⚠️ getUsers failed for ${subdomain}:`, uRes.reason.message);
    warnings.push(`Usuários indisponíveis: ${uRes.reason.message}`);
  }
  if (sRes.status === 'rejected') {
    console.error(`⚠️ detectSourceField failed for ${subdomain}:`, sRes.reason.message);
  }
