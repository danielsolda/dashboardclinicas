...
async function buildSLAData(subdomain, options = {}) {
  ...
  // ═══════════════════════════════════════════════════════
  // CLOSE DETECTION: contact → talk_created → talk_id → talk_closed
  //
  // Chain: talk_created has entity_type='contact', entity_id=contact_id
  //        and value_after contains the talk info.
  //        talk_closed has entity_type='talk', entity_id=talk_id.
  //
  // We build: contact_id → [talk_ids] → check if closed after timestamp
  // ═══════════════════════════════════════════════════════

  const closedTypes = new Set(['talk_closed', 'conversation_answered']);

  // Step 1: Build contact_id → [talk_ids] from talk_created events
  // talk_created has entity_type that varies, but entity_id should be contact or lead related
  // Also extract talk_id from value_after or from the event itself
  const contactTalks = {}; // contact_id → [{ talk_id, created_at }]

  talkEvents.filter(e => e.type === 'talk_created').forEach(e => {
    // The entity_id for talk_created is usually the contact
    const contactId = e.entity_id;
    // The talk_id might be in value_after
    let talkId = null;
    if (e.value_after?.[0]?.talk?.id) {
      talkId = e.value_after[0].talk.id;
    } else if (e.value_after?.[0]?.note?.id) {
      // Sometimes stored differently
      talkId = e.value_after[0].note.id;
    }
    if (!contactTalks[contactId]) contactTalks[contactId] = [];
    contactTalks[contactId].push({ talk_id: talkId, created_at: e.created_at, entity_type: e.entity_type });
  });

  // Step 2: Build talk_id → latest close timestamp from talk_closed/conversation_answered
  // talk_closed has entity_type='talk', entity_id=talk_id (numeric)
  const talkClosedAt = {}; // talk_id → latest close timestamp
  // Also index by entity_id for ANY close type (to catch conversation_answered which may use contact/lead)
  const entityClosedAt = {}; // entity_id → latest close timestamp (catch-all)

  talkEvents.filter(e => closedTypes.has(e.type)).forEach(e => {
    // For talk_closed: entity_type='talk', entity_id = talk_id
    if (e.entity_type === 'talk') {
      talkClosedAt[e.entity_id] = Math.max(talkClosedAt[e.entity_id] || 0, e.created_at);
    }
    // For conversation_answered: might be contact or talk
    // Store by entity_id as catch-all
    entityClosedAt[e.entity_id] = Math.max(entityClosedAt[e.entity_id] || 0, e.created_at);
  });

  console.log(`📊 Talk chain: ${Object.keys(contactTalks).length} contacts with talks, ${Object.keys(talkClosedAt).length} talks closed, ${Object.keys(entityClosedAt).length} entities closed`);

  // Step 2b: Build reverse mapping talk_id → contact_id and resolve close events to leads
  const talkToContact = {}; // talk_id → contact_id
  ...
  // Resolve close events (talk_closed, conversation_answered) to lead IDs
  // and inject them into chatByLead so Phase 1 sees them inline
  ...
  talkEvents.filter(e => closedTypes.has(e.type)).forEach(e => {
    ...
    if (e.entity_type === 'lead' && leadIds.has(e.entity_id)) {
      ...
      // talk_closed: entity_id is talk_id → reverse to contact → then to lead
  ...
  console.log(`📊 Close events injected into lead timelines: ${closeEventsInjected}`);

  // Step 3: Build lead → latestCloseTimestamp function
  // For a lead, get its contact_ids → get their talk_ids → check if closed
  ...
  function getLeadLatestClose(leadId) {
    ...
    // Get contact IDs for this lead
    const contactIds = [];
    if (lead._embedded?.contacts) {
      lead._embedded.contacts.forEach(c => contactIds.push(c.id));
    }
    // Also check the entity_ids from chat events mapped to this lead
    const chatEvts = chatByLead[leadId] || [];
    chatEvts.forEach(e => {
      if (e.entity_type === 'contact') contactIds.push(e.entity_id);
    });

    const uniqueContacts = [...new Set(contactIds)];

    for (const cid of uniqueContacts) {
      // Direct entity close (conversation_answered on contact)
      if (entityClosedAt[cid]) {
        latestClose = Math.max(latestClose, entityClosedAt[cid]);
      }

      // Chain: contact → talks → check each talk for close
      const talks = contactTalks[cid] || [];
      for (const t of talks) {
        if (t.talk_id && talkClosedAt[t.talk_id]) {
          latestClose = Math.max(latestClose, talkClosedAt[t.talk_id]);
        }
      }
    }

    // Also check direct entity_id matches for the lead itself
    if (entityClosedAt[leadId]) {
      latestClose = Math.max(latestClose, entityClosedAt[leadId]);
    }
