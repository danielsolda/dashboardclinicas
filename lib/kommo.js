...
async function fetchChatAndResponsibleEvents(subdomain, dateFrom, dateTo) {
  // Chat messages — NO entity filter (events may be on contacts or leads)
  const chatParams = {
  ...
  // Responsible changes — these ARE on leads
  const respParams = {
  ...
  console.log('📨 Fetching chat events + responsible changes (no entity filter)...');
  ...
  console.log(`📨 Chat events: ${chatEvents.length} (lead: ${chatEvents.filter(e=>e.entity_type==='lead').length}, contact: ${chatEvents.filter(e=>e.entity_type==='contact').length})`);
  console.log(`📨 Responsible changes: ${respEvents.length}`);
  ...
  for (const [leadIdStr, evts] of Object.entries(chatByLead)) {
    ...
    for (const evt of evts) {
      ...
      if (isIncoming) {
        ...
        // If multiple incoming in a row, keep the first unanswered
